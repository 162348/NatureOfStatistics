...
title=]
    ...
    Zermeloが選択公理と同値であることを示し，Zornがさらに使いやすい同値命題を付け加えた．
\end{tcolorbox}
...
\begin{lemma}
    開写像・閉写像は合成しても保たれる．2つの開写像の積は開だが，閉写像では成り立たない．
\end{lemma}
...
\begin{definition}[perfect mapping]
    連続な閉写像を\textbf{完全写像}という．
\end{definition}

\begin{proposition}[連続写像が閉であることの特徴付け]
    $f:X\to Y$が連続であるとする．
\end{proposition}
...
\begin{proposition}[連続開写像の性質]
    $f:X\to Y$を連続な開写像とする．
    \begin{enumerate}
        \item $\forall_{B\in P(Y)}\;(f^{-1}(B))^\circ=f^{-1}(U^\circ)$．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    全単射な連続関数は，開・閉写像ならば位相同型である．なお，一般の全単射が開であることと閉であることとは同値．
    また，全単射な連続写像の逆写像は，開かつ閉写像であるが，連続であるとは限らない．
\end{tcolorbox}
...
title=]
    ...
    しかし導来演算子は原初Cantor (1872)が$\R$の研究に用いた．
\end{tcolorbox}
...
\begin{proposition}[閉包による閉集合の特徴付け]
    $A\in P(X)$が閉集合であることと，$\o{A}=A$が成り立つこととは同値．
\end{proposition}
...
\begin{remarks}[触点との違い]
    ...
    そして，性質$x\in\o{A\setminus\{x\}}$は$x$が集積点であることを特徴づける(Kuratowskiの精神,\ref{thm-Kuratowski})．
    集積点でない触点とは，孤立点のみである．
\end{remarks}

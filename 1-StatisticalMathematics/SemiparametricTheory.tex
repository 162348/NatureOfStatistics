...
\begin{definition}[$P$-path]
    $\bP\in\P$に対して，$\bP$-道とは，$(P_t)_{t\in\R_+}:[0,\ep)\to\P;P_0=\bP$をいう．
\end{definition}

\begin{definition}[differentiability of path, score function]
    ($L^2$-)微分可能な$P$-道とは，$P$-道$(P_t)_{t\in[0,\ep)}$であって，
    \begin{enumerate}
        \item ある可測関数$g\in\L(\X,P)$に対して
        \[\int\paren{\frac{dP^{1/2}_t-dP^{1/2}}{t}-\frac{1}{2}gdP^{1/2}}^2\xrightarrow{t\to0}0.\]
        を満たすことをいう．このとき，$dP^{1/2}$は測度が定める線型作用素の二乗根作用素とする．
...
\begin{definition}[tangent set / space]
    $P$を通る全ての可微分な道$(P_t)_{t\in\R_+}$に関するスコア関数$g$全体の集合を，$P$における\textbf{接集合}といい，$\dot{\P}_P$\cite{vanderVaart Semiparametric Theory}または$T_P(\P)$\cite{Pfanzagl}で表す．
    ほとんどの場合，これは閉部分空間となる．
\end{definition}

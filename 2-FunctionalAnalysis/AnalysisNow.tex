...
\begin{definition}[finite rank]
    ...
    \begin{enumerate}
        \item $T$が\textbf{有限階数}であるとは，$\Im T$が$H$の有限次元部分空間であることをいう（したがって特に閉\ref{prop-finite-subspaces}）．
...
\begin{theorem}[double commutant theorem (von Neumann 1929)]
...
\begin{definition}[operator \textbf{in} a Hilbert space, domain, densely defined, extension]
    ...
    \begin{enumerate}
        ...
        \item $D(S)\subset D(T),S=T\on D(S)$が成り立つとき，$T$は$S$の\textbf{延長}であるといい，$S\subset T$で表す．
...
\begin{example}\mbox{}
    \begin{enumerate}
        \item 全域で定義されている閉作用素は有界である（閉グラフ定理）．
...
\begin{theorem}[可閉性の自己共役作用素による特徴付け]
    $T$を$H$内の稠密に定義された作用素とする．
    \begin{enumerate}
        \item $T^*$は閉作用素である．
        \item 直交分解$H\oplus H=\oo{\Graph(T)}\oplus U(\Graph(T^*))$が存在する．ただし，$U\in\Iso(H\oplus H)$は$U(z,y)=(-y,z)$によって定まるユニタリ作用素とした．
        \item $T$が可閉であることと，$T^*$が稠密に定義されていることとは同値．このとき，$\o{T}=T^{**}$すなわち$\oo{\Graph(T)}=\Graph(T^{**})$が成り立つ．
    \end{enumerate}
\end{theorem}
...
\begin{proposition}
    $T$を稠密に定義された$H$内の閉作用素で，稠密な像を持った単射とする．
    このとき，$T^*,T^{-1}$も全く同じ性質を持ち，$(T^*)^{-1}=(T^{-1})^*$が成り立つ．
\end{proposition}

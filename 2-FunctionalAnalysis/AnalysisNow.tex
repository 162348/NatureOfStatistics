...
\begin{quotation}
    完備な距離を内積が誘導する場合をHilbert空間という．些細な違いかもしれないが，理論がフルパワーを発するのはこの場合のみである．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    Hilbert空間では，内積を通じて作用素の間に対応がつく．
    これを随伴と呼ぶ．これは行列の共役転置の一般化に他ならない．
...
title=]
    ...
    まず$B(H)$を調べる方法として，距離構造に注目する．
    すると正規作用素などの性質は純粋に距離の言葉のみでも捉えられる．
\end{tcolorbox}
...
\begin{definition}[self-adjoint / hermitian]\mbox{}
    \begin{enumerate}
        ...
        \item $T\in B(H)$が$T=T^*$を満たすとき，これを自己共役作用素という．これを$B_\sa(H)=B(H)^{\Brace{*}}$と表すと，$B(H)$の閉な実部分空間となる．
    \end{enumerate}
\end{definition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    正規であるという可換性条件は，距離的な翻訳がある．
    スペクトル定理は，正規作用素は対角化可能であることを主張する．
\end{tcolorbox}

\begin{definition}[normal]
    作用素$T\in B(H)$について，次の２条件は同値．
    \begin{enumerate}
        \item $T^*T=TT^*$．
...
\begin{definition}[positive (semi-definite)]
    $T\in B(H)$が\textbf{半正定値}であるとは，$T=T^*$かつ$\forall_{x\in H}\;(Tx|x)\ge 0$を満たすことをいう．
...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \item $T_1T_2$は一般には半正定値とも自己共役とも限らない．
    \end{enumerate}
\end{lemma}
...
\begin{proposition}\mbox{}
    \begin{enumerate}
        ...
        \item この部分空間内の半正定値作用素のなす凸錐は，順序$S\le T:\Leftrightarrow T-S\ge 0$をなす．
    \end{enumerate}
\end{proposition}
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item さらに$0\le S$であるとき，$\norm{S}\le\norm{T}$．特に，$S\le I$ならば$\norm{S}\le1$．
        \item $-I\le T\le I$と$\norm{T}\le 1$は同値．
    \end{enumerate}
\end{proposition}

\begin{lemma}
    正係数を持つ多項式の列$\{p_n\}\subset\R_{>0}[x]$であって，和$\sum_{n\in\N}p_n$が$[0,1]$上$t\mapsto 1-(1-t)^{1/2}$に一様収束するものが存在する．
\end{lemma}

\begin{proposition}[square root lemma]
    半正定値な作用素$T\in B(H)$について，
    \begin{enumerate}
        \item ただ一つの半正定値な作用素$T^{1/2}$が存在して，$(T^{1/2})^2=T$を満たす．
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item $T^{-1}\ge 0$ならば，$T^{1/2}$は可逆で，$(T^{-1})^{1/2}=(T^{1/2})^{-1}$．
        \item $T\le S$ならば，$S^{-1}\le T^{-1}$．
    \end{enumerate}
\end{proposition}

\subsection{射影}

\begin{definition}[(orthogonal) projection]\mbox{}
    \begin{enumerate}
        \item 閉部分空間$X\le H$について，直交分解$H=X+X^\perp$が導く作用素$P:H\to H;y\mapsto x$は$\norm{P}\le 1$かつ$P^2=P$を満たす，半正定値作用素である．これを\textbf{直交射影}という．
...
\begin{remarks}[diagonalizable]\mbox{}
    \begin{enumerate}
        ...
        なお，この無限和は強位相については収束する（各点収束）が，そのほかは保証されない．
    \end{enumerate}
\end{remarks}
...
\begin{lemma}
    $T$が対角化可能であるとする．
    \begin{enumerate}
        \item $T^*$の固有ベクトルも$(e_j)$，固有値は$(\o{\lambda_j})$である．
        \item $TT^*=T^*T$である．すなわち，$T$は正規．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    全ての座標変換はユニタリ作用素で記述される．
    よって，この分だけ条件を緩めることに価値がある．
    部分群の系列
    \[U(H)\mono\GL(H)\mono B(H)\]
    が考えられる．
\end{tcolorbox}

\begin{definition}[unitary operator / orthogonal operator]
    $H$の自己等長同型を\textbf{ユニタリ作用素}という．実線型空間については，直交作用素ともいう．
\end{definition}
...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \item $U^*U=I$．
        \item 正規である：$UU^*=U^*U=I$．
        \item 一般の作用素$V\in B(H)$が可逆で$V^{-1}=V^*$を満たすなら，$V$はユニタリである．
    \end{enumerate}
\end{lemma}
\begin{proof}\mbox{}
    \begin{enumerate}
        \item 極化恒等式\ref{prop-polarization-identity}より，等長写像ならば内積を保つことに注意．
        \item $(Ux|Uy)=(x|U^*Uy)=(x|y)$より，補題\ref{lemma-correspondence-between-sesquilinearform-and-operator}から，$U^*U=I$．
    \end{enumerate}
\end{proof}
...
\begin{definition}[partial isometry, initial subspace, final subspace]
    作用素$U\in B(H)$が\textbf{部分等長作用素}であるとは，閉部分集合$X$が存在して，制限$U|_X$は等長写像で，$U|_{X^\perp}=0$を満たすことをいう．
    $X$を\textbf{初期部分空間}，$\Im U$を最終部分空間という．
\end{definition}
\begin{lemma}\mbox{}
    \begin{enumerate}
        \item このとき，$X^\perp=\Ker U,X=\dbloverline{(\Im U^*)}$である．
        \item $P:=U^*U$とおくと，$\forall_{x\in X}\;Px=x,\;\forall_{x^\perp\in X^\perp}\;Px^\perp=0$．すなわち，$P$は射影である．
        \item (2)は部分等長作用素を特徴付ける．すなわち，作用素$U\in B(H)$について$U^*U$が射影となるとき，$U,U^*$は部分等長作用素である．$U^*$を$U$の\textbf{部分逆}(partial inverse)という．
    \end{enumerate}
\end{lemma}

\begin{example}[unilateral shift operator]
    $S$は等長作用素だが，ユニタリーではない（すなわち全射でない）例となっている．
\end{example}

\begin{theorem}[polar decomposition (von Neumann)]\mbox{}
    \begin{enumerate}
        \item 任意の作用素$T\in B(H)$に対して，ただ一つの半正定値作用素$\abs{T}\in B(H)$が存在して，$\forall_{x\in H}\;\norm{Tx}=\norm{\abs{T}x}$を満たす．
        \item $\abs{T}=(T^*T)^{1/2}$である．
        \item ただ一つの部分等長写像$U$が存在して，$\Ker U=\Ker T,U\abs{T}=T$を満たす．特に，$U^*U\abs{T}=U^*T=\abs{T},\;UU^*T=T$である．
    \end{enumerate}
\end{theorem}

\begin{proposition}
    $T\in B(H)$が可逆であるとき，部分等長写像$U$はユニタリである．
\end{proposition}
...
\begin{lemma}
    $T=T^*$かつ$\norm{T}\le 1$ならば，作用素$U:=T+i(I-T^2)^{1/2}$はユニタリであり，$T=\frac{1}{2}(U+U^*)$である．
\end{lemma}
...
\begin{definition}[numerical radius]
    ...
    \[\nnorm{T}=\sup\Brace{\abs{(Tx|x)}\in\R\mid x\in H,\norm{x}\le 1}\]
    をいう．
\end{definition}
...
\begin{theorem}[multiplication operator and its symbol]
    $(X,\Om,\mu)$を$\sigma$-有限な測度空間とし，$H:=L^2(X,\Om,\mu)=:L^2(\mu)$とする．任意の$\varphi\in L^\infty(\mu)$に対して，post-compositionの定める写像
    \[\xymatrix@R-2pc{
        M_\varphi:L^2(\mu)\ar[r]&L^2(\mu)\\
        \rotatebox[origin=c]{90}{$\in$}&\rotatebox[origin=c]{90}{$\in$}\\
        f\ar@{|->}[r]&\varphi\cdot f
    }\]
    は，$M_\varphi\in B(L^2(\mu))$を満たし，$\norm{M_\varphi}=\norm{\varphi}_\infty$を満たす．
    $\varphi$を\textbf{乗算作用素}$M_\varphi$の\textbf{記号}という．
\end{theorem}

\begin{theorem}[integral operator / transform and its kernel]
...
\begin{example}[Volterra operator]
    $k:[0,1]\times[0,1]\to\R$を集合$\{(x,y)\in[0,1]\times[0,1]\mid y<x\}$の特性関数とする．
    これを核とする積分作用素$V:L^2(0,1)\to L^2(0,1);f\mapsto Vf(x)=\int^1_0k(x,y)f(y)dy=\int^x_0f(y)dy$をVolterra作用素という．
    これは不定積分としての積分作用素である．\footnote{ヴォルテラ積分方程式は、人口学や、粘弾性物質の研究、保険数学に現れる再生方程式などへと応用されている。}
\end{example}
...
\begin{example}[unilateral shift]
    \[\xymatrix@R-2pc{
        S:l^2\ar[r]&l^2\\
        \rotatebox[origin=c]{90}{$\in$}&\rotatebox[origin=c]{90}{$\in$}\\
...
\begin{example}[随伴]\mbox{}
    \begin{enumerate}
        \item 乗算作用素については$M^*_\varphi=M_{\o{\varphi}}$となる．
        \item $k$を核とする積分作用素$K$については$K^*$は$k^*(x,y)=\o{k(y,x)}$を核とする積分作用素となる．
        \item shift作用素は方向が逆になる：$S^*(\al_1,\al_2,\cdots)=(\al_2,\al_3,\cdots)$．これを\textbf{後方シフト(backward shift)}と呼ぶ．
    \end{enumerate}
\end{example}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    （$X$が局所コンパクトハウスドルフ空間である時，）$C_c(X)\subset C_b(X)$の関係と$B_f(H)\subset B(H)$の関係は非常に似ている．
    These classes describe local phenomena on $H$ and on $X$.\cite{AnalysisNow}
    そこで，$C_c(X)$の完備化として得た$C_0(X)$に対応するクラスを，作用素論でも構成することを考える．
\end{tcolorbox}

...
\subsection{Banach空間値積分}\label{subsection-Bochner-integral}
...
\begin{discussion}[双対ペアと微分]
    Bochner積分論\ref{subsection-Bochner-integral}と同様に，双対ペアは微分の定義を一意に示唆する．
    部分積分の公式
...
title=]
    ...
    Bochner積分\ref{subsection-Bochner-integral}という．
\end{tcolorbox}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item $\R_+$を係数とした線型和のことを，\textbf{錐結合}という．錐結合全体の集合は閉凸錐になり，これを生成される錐という．$\Cone S$で表す．
    \end{enumerate}
\end{definition}
...
\begin{definition}[convex function, strictly convex function]
    ...
    $u=v$または$\lambda\in\partial[0,1]$である場合を除き，常に不等号が真に成り立つとき，$F$は\textbf{厳密に凸}であるという．
\end{definition}
\begin{remarks}[effective domain, proper]
    定義域に$+\infty$を許すことで，任意の部分関数を，$V$全体に凸なまま延長して考えることができる．
    $\dom F:=\Brace{u\in V\mid F(u)<\infty}$を\textbf{有効定義域}という．
    ...
    以降，真の凸関数といったとき，$F$は$-\infty$を取らず，恒等的に$+\infty$ではないとする．
\end{remarks}
...
\begin{definition}
    ある連続線型汎関数$l\in V^*$と実数$\al$を用いて$l+\al$と表せ，さらに有界性$\forall_{v\in V}\;v>l(v)+\al$
    を満たす関数を\textbf{連続affine関数}という．
...
\begin{proposition}[$\Gamma$-正則化のエピグラフ]
...
\begin{definition}[primal problem, dual problem]
    $\inf\norm{u-u_0}=\al,u\in L$を主問題とする．これに対して

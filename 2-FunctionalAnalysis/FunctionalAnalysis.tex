...
\begin{theorem}[実数上のRadon積分のLebesgue-Stieltjes積分としての表現\ref{thm-Stieltjes}]
    正な線型汎関数$\int\in (C_c(\R))^*_+$と，Radon測度$\RM(X;[0,\infty])$とについて，
    \[(C_c(\R))^\vee_+\simeq_\Set\Brace{m\in\Map(\R,\R)\mid m\text{は単調増加で下半連続}}\]
\end{theorem}
...
\begin{theorem}[Riesz-Markov-Kakutani (1941)]
    $X$をコンパクトハウスドルフ空間，$M(X)$をBaire測度が全変動ノルムについてなすBanach空間とする．
...
\begin{theorem}[markov-Alexandorff (1940)]
    $X$を$T_4$-位相空間，$M_f(X)$をその上の内部正則なBorel有限加法的符号付き測度が全変動ノルムについてなす空間とする．
    次の対応は等長同型$(C_b(X))^*\simeq_\Ban M_f(X)$を定める：
    \[\xymatrix@R-2pc{
        (C_b(X))^*\ar[r]&M_f(X)\\
        \rotatebox[origin=c]{90}{$\in$}&\rotatebox[origin=c]{90}{$\in$}\\
        L\ar@{|->}[r]&L(f)=\int_Xfd\mu
    }\]
\end{theorem}
\begin{history}
...
\begin{definition}[Lebesgue space]
    実数$p\in[1,\infty)$について，位数$p$のLebesgue空間とは，線型空間
...
\begin{proposition}\label{prop-dense-subset-of-Lp}
    $p\in[1,\infty)$とする．$C_c(X)$（の商写像についての像）は$L^p(X)$上稠密である．
\end{proposition}
\begin{remarks}

...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    古典的Wiener空間$W_0$は$0$から始まる連続な見本過程$R_+\to\R$全体の空間である．
    ここには標準的な位相が1つ定まる．
\end{tcolorbox}
...
\begin{discussion}[Poisson過程の構成]
    ...
    独立にパラメータ$\lambda$の指数分布に従う$\R_+$-値確率変数列$(S_j)$を取る：$P[S_j\ge t]=e^{-\lambda t}$．
...
\begin{definition}[diffusion process]
    確率過程$(X_t)_{t\in\R_+}$が$P_x[X_0=x]=1$と次の条件を満たすとき，$\M:=(\M_x)_{x\in\R},\M_x:=\Brace{X_t(\om)\in\R\mid t\in\R_+,\om\in(C,P_x)}$を\textbf{一様な連続強Markov過程}または\textbf{拡散過程}という．
    \begin{quote}
...
\begin{definition}[advection]\mbox{}
    \begin{enumerate}
...
\begin{definition}[Rademacher distribution, Rademacher / Steinhaus variable]\mbox{}
    \begin{enumerate}
        \item $\Z$上の確率質量関数

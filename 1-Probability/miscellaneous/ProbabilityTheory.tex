...
\subsection{事象の公理(von Mises)}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=「集合の公理」のもう一つの見方]
    集合を事象と翻訳した「標本空間(Merkmalraum)」の概念はvon Misesの21から31までの仕事による．
...
\begin{definition}[sample space, sample, event]
    ...
    \begin{enumerate}
        ...
        \item $\F$の元を\textbf{事象}と呼ぶ．\footnote{部分集合を条件や関係だけでなく事象と捉えるのは，集合論の初歩でもやるようだ．}事象としての$\Omega\in\calF$を全事象という．
    \end{enumerate}
\end{definition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=エントロピーとは確率不変量である]
知識の限界のモデルである
確率空間には保存量が定義でき，系の時間発展を見るエルゴード理論や通信の限界を考える情報理論で用いられる．
実は，確率論で直積測度を考えるときは，暗黙にエントロピー最大の原理を仮定している．
\footnote{Entropy is a measure of disorder, given by the amount of information necessary to precisely specify the state of a system.\url{https://ncatlab.org/nlab/show/entropy}}
\end{tcolorbox}
...
\section{確率不等式まとめ}\label{sec-event}
...
\begin{theorem}[Borel測度の正則性についての結果]
    局所コンパクトハウスドルフ空間$X$上のRadon測度は有限ならば外部正則である．
\end{theorem}
...
\begin{theorem}[Riesz-Markov-Kakutani (1941)]
    $X$をコンパクトハウスドルフ空間，$M_B(X)$をBaire測度が全変動ノルムについてなすBanach空間とする．
...
\begin{theorem}[Markov-Alexandorff (1940)]
...
\begin{history}
    ...
    Alexandorff (1940)でchargeと呼んでいるものは，有限加法的な符号付き測度で，有限加法的な集合関数である．
...
\begin{tbox}{red}{$w^*$-位相と呼んでも確率測度についてはwell-definedであるが，点列の収束としてはやはり別物}
    ...
    そこで収束としては「弱収束」と「漠収束」と呼び分けることとなる．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $P(X)$の弱位相＝$\sigma(M(X),C_b(X))$-位相の性質が重要になってくるので，ここにまとめることとする．
    \begin{enumerate}
        \item $X$が可分であることと，$P(X)$が可分で距離化可能であることは同値．
        \item $X$が可分ならば，その稠密部分集合$Y\subset X$上の有限の台を持つ確率測度の全体$F(X)$が$P(X)$上稠密である．特に，Dirac測度の埋め込み$D\mono P(X)$の凸包は稠密である(Krein-Smulianの定理の系)．
        \item $X$がコンパクトは距離空間であるとき，$P(X)$もコンパクトで距離付け可能．
        \item $X$が完備可分ならば，$P(X)$も完備可分である．
        \item $X$が完備可分な距離空間であるとき，$P(X)$の部分集合が相対コンパクトであることと一様に緊密であることとは同値(Prokhorov)．
    \end{enumerate}
\end{tcolorbox}
...
\begin{theorem}[Prokhorov metric]
    $S$を完備可分距離空間とする．
...
\begin{lemma}
    デルタ測度への対応$i:X\mono P(X)$は位相の埋め込みである．
\end{lemma}

\begin{lemma}
    $\Delta:=\Im i$は$P(X)$内で点列コンパクトである．\footnote{一般に部分集合$A\subset X$の閉包$\o{A}$は点列閉包$B$を含むが，点列閉包と一致するのは一般に$X$が距離空間の場合のみ．}
\end{lemma}
...
\begin{theorem}
    $S$を可分とする．$\P\subset P(S)$について，次の2条件は同値．
    \begin{enumerate}
        \item $\P$は相対(点列)コンパクトである．
        \item 距離$\pi$について相対コンパクトである．
    \end{enumerate}
\end{theorem}
...
\begin{theorem}[Prokhorov：緊密性とは，相対コンパクト性の特徴付けである]\label{thm-Prokhorov}
    ...
    $X$が完備可分であるとき，(2)$\Rightarrow$(1)も成り立つ．
    \begin{enumerate}
...
\section{距離空間上の有限測度の弱収束}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
    title=Banach空間論・位相空間論の発展を後押しした研究対象]
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $S$上の開集合上の関数として，または積分として，双対的な2つの見方のどちらでも良いが，関数解析的にとらえて位相の議論に持ち込むのが良いだろう．
\end{tcolorbox}

\begin{theorem}
    距離空間$(S,\B(S))$上の任意の確率測度は位相的に正則，特にRadonである：
    \[\forall_{B\in\B(S)}\quad P(B)=\sup_{F\subset B;F:closed}P(F)=\inf_{B\subset G\osub S}P(G).\]
    特に，距離空間上の確率測度は，開または閉集合上で値が決まれば，一意に定まる．
\end{theorem}
\begin{remarks}
    また，確率測度$P$が$S$上緊密であることは，任意のBorel集合$B\in\B(S)$が$P$に関して緊密(=内部正則)であれば良い．
\end{remarks}
...
\begin{definition}[weak convergence of measure, vague convergence]
    ...
    このとき考え得る$\sigma(M(X),C_b(X))$-位相に関する収束を\textbf{弱収束}，$\sigma(M(X),C_0(X))$-位相に関する収束を\textbf{漠収束}という．
\end{definition}
\begin{remarks}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item $\forall_{f\in UC_b(S)}\;Pf=Qf$．
        \item $P=Q$．
    \end{enumerate}
\end{lemma}
...
\begin{theorem}[３つ目の特徴付け]
    列$\{P_n\}\subset P(S)$と$P\in P(S)$について，次の2条件は同値：
    \begin{enumerate}
        \item $P_n\Rightarrow P$．
        \item $\{P_n\}\subset P(S)$は相対コンパクト集合．
    \end{enumerate}
\end{theorem}
...
\begin{theorem}
    $S$が次を満たすならば，任意の$S$上の確率測度は緊密である．
    \begin{enumerate}
        \item $S$は完備で可分．
...
\begin{proposition}[連続写像は弱連続作用素を引き起こす]
    $\nu_n\Rightarrow\nu$かつ$T:S_1\to S_2$が連続ならば，
    $T_*\nu_n\Rightarrow T_*\nu^T$．
\end{proposition}

\begin{theorem}[連続写像定理]
    $h:S_1\to S_2$を可測写像，$D_h\subset S$を不連続点とすると，$D_h\in\B(S)$．
    このとき，$P_n\Rightarrow P$かつ$P[D_h]=0$ならば，$h_*P_n\Rightarrow h_*P$．
\end{theorem}
...
\subsection{平均収束列の一様可積分性}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    先程は$f_n$の$L^1$ノルムの収束で特徴付けた．
    今回は，$f_n$が定める不定積分が，集合関数$\A\to\R$として各点収束するならば一様可積分であることを論じる．
\end{tcolorbox}
...
\begin{theorem}
    $X,X_n\subset L^1(S)$を非負関数とする．$X_n\Rightarrow X$かつ$E[X_n]\to E[X]$ならば，$\{X_n\}$は一様可積分．
\end{theorem}

\subsection{概収束列の一様可積分性}
...
\subsection{確率変数のなす空間}

\begin{proposition}
    $X_1\in\L(\Om;S_1),X_2\in\L(\Om;S_2)$と，$(X_1,X_2)\in\L(\Om;S_1\times S_2)$は同値．
    しかし，$S_1\times S_2$が可分でない限り，$X_1,X_2$が確率変数であっても，$(X_1,X_2)$も確率変数とは限らない．
\end{proposition}

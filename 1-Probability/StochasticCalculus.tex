...
\begin{proposition}[一般化された独立同分布列の存在定理：ホワイトノイズ]
    $H$を可分実Hilbert空間とする．
...
\begin{proposition}[white noiseはGauss系である]\mbox{}
    \begin{enumerate}
...
\begin{lemma}[自然な情報系の特徴付け]\label{lemma-Blumenthal}
    ...
    \begin{enumerate}
        \item (左連続性) このBrown運動の自然な情報系$(\F_t)$について，$\F_t$は柱状集合$C(t_1,\cdots,t_n;I_1,\cdots,I_n)\;(0\le t_1<\cdots<t_n\le t)$の全体によって生成される．また，$t_n<t$としても変わらない．
        \item ($0$における右連続性) また$\F_t$はBrown運動の族$(B_{t+h}-B_h)_{t\in\R_+,h>0}$の柱状集合$D(t_1,\cdots,t_n;h;I_1,\cdots,I_n)\;(h>0)$の全体によっても生成される：

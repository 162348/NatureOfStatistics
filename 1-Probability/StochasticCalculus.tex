...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    酔歩のスケールとして得られるこの方法は，Brown運動が基本的な対象であることの直感的な説明となる．
...
\begin{discussion}
    任意の$T>0$に対して，これを$n\in\N$等分する考え方で，独立同分布に従う$n$個の確率変数$\xi_1,\cdots,\xi_n\sim(0,T/n)$を考える．
    この和$R_k:=\sum_{i=1}^k\xi_i\;(k\in[n])$を考えると，過程$(R_k)_{k\in[n]}$は$n$ステップの酔歩で，分散が$T/n$である．
    この過程について，$n\to\infty$の極限を取ることを考える．すると，ステップ数は増え，ステップの幅は小さくなる．
    このとき，中心極限定理より，列$(R_k)_{k\in\N}$は正規分布$N(0,T)$に分布収束する．
    実際，$X_i:=\sqrt{n}\xi_i\sim(0,T)$とすると$R_k=\frac{1}{\sqrt{n}}\sum^k_{i=1}X_i$となる．

    続いて，$\forall_{k=0,\cdots,n}\;S_n\paren{\frac{kT}{n}}=R_k$を満たす$S_n:[0,T]\to\R$を，線型補間による連続延長とする．
...
\begin{example}[geometric Brownian motion]
    他の確率過程として，
    \[\exp\paren{\beta t-\paren{\frac{\al^2t}{2}}+\al W_t}\]
    を幾何ブラウン運動という．
\end{example}

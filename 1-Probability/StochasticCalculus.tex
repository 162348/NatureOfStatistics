...
\begin{proof}\mbox{}
    \begin{description}
        ...
        \item[(2)$\Rightarrow$(1)]
        \begin{enumerate}[({B}1)]
            \item 平均が$0$であることより，特に$m(0)=E[B_0]=0$．よって，$B_0=0\;\as$
...
\begin{shishin}
    $T>0$として，区間$[0,T]$上の過程$B=(B_t)_{t\in[0,T]}$を条件(B1)から(B4)を満たすように構成すれば，$T=1$とした場合について，
    過程の列$(Y^{(i)})_{i\in\N}$を「つなげた」過程を
    \[W_t:=\paren{\sum^{\floor{t}-1}_{i=0}Y_1^{(i)}}+Y^{[t]}_{t-[t]}\]
    と定めると，これが目標のBrown運動となる．
    よって以降の構成の議論では，有界区間$[0,T]$に注目して議論する．
\end{shishin}

\subsection{Gauss過程としての構成}

\begin{theorem}[Kolmogorov's continuity criterion / 正規化定理]\label{thm-Kolmogorov-continuity-criterion}
    実過程$X=(X_t)_{t\in[0,T]}$が$\exists_{\al,\beta,K>0}\;\forall_{s,t\in[0,T]}\;E[\abs{X_{t}-X_s}^\beta]\le K\abs{t-s}^{1+\al}$を満たすならば，$X$のある修正$\wt{X}$が存在して，
    任意の$\gamma<\frac{\al}{\beta}$について次の2条件を満たす．
    \begin{enumerate}
        \item $\exists_{G_\gamma\in\Meas(\Om',\R)}\;\forall_{s,t\in[0,T]}\;\abs{\wt{X}_t-\wt{X}_s}\le G_\gamma\abs{t-s}^\gamma$．
        \item $\wt{X}$の見本道は$\gamma$-次Holder連続である．
    \end{enumerate}
\end{theorem}
\begin{remarks}
    Kolmogorovの拡張定理により存在が証明される，平均$0$共分散$\Gamma(s,t)=\min(s,t)$のGauss過程は，この十分条件を満たす．
\end{remarks}
...
title=]
    ...
    また，LevyとCiesielskiは基底としてウェーブレットの一例であるHaar関数系を取った．
\end{tcolorbox}
...
\begin{definition}[skelton / Cameron-Martin subspace]
    \[H:=\Brace{w(t)=\int^t_0h(x)dx\in C_{(0)}([0,1])\;\middle|\;h\in L^2([0,1],dx)}\]
    と定めると，これは内積$(f_1,f_2):=\int^1_0h_1h_2dx$についてHilbert空間となる．
    これを$W_0$の骨格，またはCameron-Martin部分空間という．
\end{definition}

\begin{remark}
    明示的にHaar基底を使わずとも構成できる．
...
\section{Cameron-Matrin定理}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    ドリフト付きのBrown運動を調べることは，標準Brown運動を調べることに等しい．
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
    ...
    これが定める積分をうまく定義すると，Stieltjes積分の状況がうまく模倣出来る．
    ホワイトノイズが積分にあたり，Brown運動が有界変動関数に当たる．
\end{tcolorbox}
...
\begin{definition}[Wiener integral]\label{def-Wiener-integral}
...
\begin{definition}[white noise]
    $D\subset\R^m$をBorel集合とする．$D$上の\textbf{ホワイトノイズ}とは，$D$内の測度確定な可測集合
    $\D:=\Brace{A\in\B(\R^m)\cap P(D)\mid l(A)<\infty}$によって添字付けられた
    中心化されたGauss確率変数の族$W:=\{W(A)\}_{A\in\D}$であって，
    Lebesgue測度$l$に関して$E[W(A)W(B)]=l(A\cap B)$を満たすものをいう．
\end{definition}
\begin{remarks}
    この族$W:\D\ni A\mapsto W(A)$は，線型な等長写像$L^2(D)\ni\varphi\mapsto\int_D\varphi(x)W(dx)\in\o{W}\subset L^2(\Om)$に延長出来る．
    定義\ref{def-white-noise-1}ではこちらを，Wiener積分の言葉を使わずに描写することでホワイトノイズの定義とした．
...
\begin{proposition}
    $W$を$\R_+$上のホワイトノイズとすると，$W_t:=W([0,t])=W_{1_{[0,t]}}$と定めるとこれはBrown運動である．
\end{proposition}
...
\begin{proposition}[$\sigma$-代数の特徴付け]
    $(\Om,\F,P)$をWiener空間とする．
    $\F$は次の円筒集合の集合$\cC\subset P(\Om)$によって生成される：

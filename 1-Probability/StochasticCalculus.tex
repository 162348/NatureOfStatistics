...
\end{proof}
\begin{remarks}
    Wiener空間$C(\R_+)$などの無限次元位相線型空間は局所コンパクトになり得ない．
...
\begin{proof}
    ...
    \[u^{(n)}_t=\sum^{N^{(n)}-1}_{j=0}\phi_j^{(n)}1_{(t_j^{(n)},t_{j+1}^{(n)}]}(t)\]
    とする．
    \begin{enumerate}
        \item 単過程
        \begin{align*}
        ...
        \end{align*}
        \begin{align*}
            E[I(u)]&=E\Square{\lim_{n\to\infty}\sum^{N^{(n)}-1}_{j=0}\phi_j^{(n)}(B_{t_{j+1}^{(n)}-t_j^{(n)}})}\\
            &=\lim_{n\to\infty}E\Square{\int^\infty_0u_t^{(n)}dB_t}=0.
        \end{align*}
    \end{enumerate}
\end{proof}
